...
- If OpenDota API requests are failing 100% of the time, this option can be switched from the default `auto` to `ipv4` to try and resolve the issue. For more specific information, please see [OpenDota Request Failed](./opendota-failed.md).
...
#### fontPath `string`
- Directory path for specific font files. Default is `data/fonts/dota2tracker`.
- The plugin automatically scans this directory for font files and generates selectable options in the `fonts` configuration below.
- Supported font formats: `.ttf`, `.otf`, `.woff`, `.woff2`, `ttc`, `sfnt`.

#### fonts
Template font settings. See [Template Fonts](./template-fonts.md) for details.
##### fonts.sans `dynamic`
- Sans-serif fonts, used as the default body text in most templates.
##### fonts.serif `dynamic`
- Serif fonts, used as title text in some templates.
##### fonts.mono `dynamic`
- Monospace fonts, used for code and numbers in some templates.
